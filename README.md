...
└── openclaw-secretary    ← Memory distillation, daily logs, ticket management
```

## Core Principles

- **Obsidian is the source of truth** — all original content lives in Obsidian
- **Notion is the publishing layer** — selected content gets published, restructured for readers
...
### `knowledge-ops`
Top-level rules for the dual Obsidian + Notion system. Defines the principle that Obsidian is the origin, Notion is publish-only, and publishing means restructuring — not copying. Takes precedence when other skills conflict.
...
### `openclaw-secretary`
The AI's own assistant. Handles MEMORY.md distillation, daily log writing, and ticket management. Without this skill, memory management has to be done manually.
